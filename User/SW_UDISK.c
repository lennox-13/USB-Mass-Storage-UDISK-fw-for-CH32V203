...
__attribute__ ((aligned(4))) static uint8_t SD_RD_BUF[DEF_UDISK_SECTOR_SIZE];
__attribute__ ((aligned(4))) static uint8_t SD_WR_BUF[DEF_UDISK_SECTOR_SIZE];
/******************************************************************************/
/* INQUITY */
 ...
 * Description    : UDISK upload one packet from actual SD sector
 *******************************************************************************/
void UDISK_Up_OnePack( void )
{
    uint8_t *pbuf;

    /* Read sector begining 512 B in to cache */
    if( UDISK_Sec_Pack_Count == 0x00 )
    {
        if (SD_ReadBlocks(UDISK_Cur_Sec_Lba, SD_RD_BUF, 1) != 0)
        {
            /* read error */
            UDISK_CMD_Deal_Status( 0x03, 0x11, 0x01 ); /* MEDIUM ERROR (Read) */
            Udisk_Transfer_Status &= ~DEF_UDISK_BLUCK_UP_FLAG;
            return;
        }
    }

    /* Grab 64B (DEF_UDISK_PACK_64) from cache */
    pbuf = &SD_RD_BUF[ (uint32_t)UDISK_Sec_Pack_Count * UDISK_Pack_Size ];
    ...
    /* No more data to send?, clear UP flag */
 ...
 * Description    : UDISK download one packet to 512B buffer and write to SD
 *******************************************************************************/
void UDISK_Down_OnePack( uint8_t *pbuf, uint16_t packlen )
{
    /* Copy packet to 512B buffer (offset order sector) */
    memcpy(&SD_WR_BUF[ (uint32_t)UDISK_Sec_Pack_Count * UDISK_Pack_Size ], pbuf, UDISK_Pack_Size);

    UDISK_Sec_Pack_Count++;
    UDISK_Transfer_DataLen -= UDISK_Pack_Size;

    /* If buffer is full (512 B), write sector to SD Card */
    if( UDISK_Sec_Pack_Count == ( DEF_UDISK_SECTOR_SIZE / UDISK_Pack_Size ) )
    {
        if (SD_WriteBlocks(UDISK_Cur_Sec_Lba, SD_WR_BUF, 1) != 0)
        {
            /* write error */
            UDISK_CMD_Deal_Status( 0x03, 0x0C, 0x01 ); /* MEDIUM ERROR (Write) */
            Udisk_Transfer_Status &= ~DEF_UDISK_BLUCK_DOWN_FLAG;
            return;
        }
